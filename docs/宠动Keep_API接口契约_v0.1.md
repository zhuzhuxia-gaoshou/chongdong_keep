...
| 项 | 约定 |
|---|---|
| BaseURL | 开发/测试/生产三环境地址由后端提供后填入此处：dev=`____` test=`____` prod=`____` |
...
| # | 方法 | 路径 | 说明 | 鉴权 | 优先级 | 状态 |
|---|---|---|---|---|---|---|
| 1 | GET | `/api/v1/ping` | 健康检查 | ❌ | P0 | 草稿 |
| 2 | POST | `/api/v1/auth/sms-code` | 发送短信验证码 | ❌ | P0 | 草稿 |
| 3 | POST | `/api/v1/auth/login` | 登录（换 token 对） | ❌ | P0 | 草稿 |
| 4 | POST | `/api/v1/auth/refresh` | 刷新 accessToken | ❌ | P0 | 草稿 |
| 5 | POST | `/api/v1/auth/logout` | 登出（作废 refreshToken） | ✅ | P0 | 草稿 |
| 6 | GET | `/api/v1/users/me` | 我的资料 | ✅ | P0 | 草稿 |
| 7 | PATCH | `/api/v1/users/me` | 改昵称/头像 | ✅ | P0 | 草稿 |
| 8 | GET | `/api/v1/pets` | 宠物列表 | ✅ | P0 | 草稿 |
...
| 15 | GET | `/api/v1/exercise-records` | 查询运动记录（分页） | ✅ | P0 | 草稿 |
